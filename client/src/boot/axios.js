      ...
      if (data) {
        ...
        if (data[0].status === 422) {
          Notify.create({
            ...
            textColor: 'white',
            icon: 'warning',
            message: 'Correo ya registrado en el sistema' /* + data[0].message */,
            position: 'center'
          })
        ...
        if (data[0].status === 423) {
          Notify.create({
            ...
            textColor: 'white',
            icon: 'warning',
            message: 'Rif ya se encuentra registrado en el sistema' /* + data[0].message */,
            position: 'center'
          })
          // return Promise.reject(data.response.data.error)
        }
        if (data[0].status === 424) {
          Notify.create({
            ...
            textColor: 'white',
            icon: 'warning',
            message: 'Por favor verifica tu conexión de internet, ha ocurrido un error en el evnio del correo' /* + data[0].message */,
            position: 'center'
          })
          // return Promise.reject(data.response.data.error)
        }
